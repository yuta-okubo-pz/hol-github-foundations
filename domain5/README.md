...
- １. 任意のリポジトリ（ドメイン1で作成した`foundations-hands-on-1`リポジトリなど）のナビゲーションのProjectsを選択する  
...
- ２. テンプレートが複数あることを確認する
- ３. ここでは、「Start from scratch」から「Table」を選択する  
  ![start from scratch](../image/image5-2.png)
- ４. 「Project name」に`foundations test`と入力し、「Create project」ボタンを選択する
- ５. ドメイン4で作成したリポジトリ`hol-javascript-calculator`でIssueを2つ作る
...
- １０. ProjectのBoardレイアウトのビューで、アイテムを「Done」に移動する
...
- ４０. 既存のProjectをコピーして、テンプレートを新しく作成する
...
- １. 任意の画面でアカウントアイコンをクリックし、`Settings`>`Code, planning, and automation`>`Saved replies`から返信テンプレートを作ってみる  
  ![create saved replies](../image/image5-13.png)
- ２. Issue/Pull requestに、saved repliesを使って返信してみる  
